    ...
    if n_comp == 1:
        ...
        file.write(
            "pkpd.two_compartments(\"{}\", {}, \"{}\", {}) \n".format(
                comp_list[0]["name"], comp_list[0]["size"],
                comp_list[1]['name'], comp_list[1]["size"]
            )
        )
    elif n_comp == 3:
        file.write(
            "pkpd.three_compartments(\"{}\", {}, \"{}\", {}, \"{}\", {}) \n".format(
                comp_list[0]["name"], comp_list[0]["size"],
                comp_list[1]['name'], comp_list[1]["size"],
                comp_list[2]['name'], comp_list[2]["size"],
            )
        )
    elif n_comp > 3:
...
def dose(file, drug_name, drug_dose, dosing, dosing_kwargs):
    file.write("##  Drug & Dose  ##\n")
    file.write("pkpd.drug_monomer(\"{}\") \n".format(drug_name))
    dose_line = "{}({}, {}".format(dosing_strategies[dosing], drug_name, drug_dose)
...
def distribution(file, drug_name, distributions):
    ...
    for dist in distributions:
        file.write("pkpd.distribute({}, {}, {}, klist=[{}, {}]) \n".format(drug_name, *dist))
    file.write("\n")
